...
return original;},load:function(file,asObject){var request=new XMLHttpRequest();request.open('GET','/'+file,false);request.send();return(asObject===false)?request.responseText:{src:request.responseText,path:file};},require:function(file){if(this._loaded[file]){return;}
if([].indexOf&&this._current.indexOf(file)!==-1){console.log('Circular dependency: '+this._current.join(' -> ')+' -> '+file);return;}
this._current.push(file);var code=this.load(this.path+file,false)+"\n //# sourceURL="+this.path+file;try{eval.call(window,code);}
catch(error){console.error(error,error.message,error.lineNumber,this.path+file);}
this._current.pop();this._loaded[file]=true;},_cachedRegexp:{escapeQuotes:/'/g,escapeBackslashes:/\\/g,unescapeQuotes:/\\'/g,removeWhitspace:/[\r\t\n]+/g,printEscaped:/\{\{([\w\.\,\(\)\[\]\"]+?)\}\}/mg,print:/\{([\w\.\,\(\)\[\]\"]+?)\}/g,js:/<\?js(.*?)\?>/mg},compileTemplate:function(t){var path=t.path||'unknown';t=t.src||t;try{var re=this._cachedRegexp;var compiled=new Function("data","try {"+"data = data || {};"+"var _parts = [];"+"var print = function(a){ _parts.push(a); };"+"with(data){ _parts.push('"+
...
return f;},getLocation:function(){return this.getURL().split(':')[0];},getFragment:function(){return this.getURL().split(':')[1]||'';},navigateTo:function(location,mode){this._navigateSilent=(mode==p.NAVIGATE.SILENT);if(this._hasPushState){var url='/'+location;window.history.pushState({},document.title,url);this._dispatch(null,mode==p.NAVIGATE.FORCE);}
else{var url=document.location.href.replace(/#.*$/,'')+'#'+location;document.location.assign(url);}
if(CONFIG.ANALYTICS.ENABLED&&window.ga){ga('send','pageview',{page:location});}},navigateToPopStack:function(){if(!this._navigationStack.length){this.navigateTo('');}
...
$(document).scrollTop(0);if(this.onsetview){this.onsetview(this.currentView);}
...
Date.MONTH_NAMES[this.getMonth()]+" "+
this.getFullYear();};"use strict"
Number.prototype.map=function(istart,istop,ostart,ostop){return ostart+(ostop-ostart)*((this-istart)/(istop-istart));};Number.prototype.limit=function(min,max){return Math.min(max,Math.max(min,this));};Number.prototype.round=function(precision){precision=Math.pow(10,precision||0);return Math.round(this*precision)/precision;};Number.prototype.floor=function(){return Math.floor(this);};Number.prototype.ceil=function(){return Math.ceil(this);};Number.prototype.toInt=function(){return(this|0);};Number.zeroes='000000000000';Number.prototype.zeroFill=function(d){var s=this.toString();return Number.zeroes.substr(0,d-s.length)+s;};"use strict";Array.prototype.erase=function(item){for(var i=this.length;i--;){if(this[i]===item){this.splice(i,1);}}
return this;};Array.prototype.random=function(){return this[Math.floor(Math.random()*this.length)];};Array.prototype.shuffle=function(){for(var j,x,i=this.length;i;j=Math.floor(Math.random()*i),x=this[--i],this[i]=this[j],this[j]=x);return this;};[].indexOf||(Array.prototype.indexOf=function(a,b,c){for(c=this.length,b=(c+~~b)%c;b<c&&(!(b in this)||this[b]!==a);b++);return b^c?b:-1;});Array.prototype.first=function(){return this.length?this[0]:null;};Array.prototype.last=function(){return this.length?this[this.length-1]:null;};"use strict";Function.prototype.bind=Function.prototype.bind||function(bind){if(typeof this!=="function"){throw new TypeError("Function.prototype.bind - what is trying to be bound is not callable");}
var aArgs=Array.prototype.slice.call(arguments,1),fToBind=this,fNOP=function(){},fBound=function(){return fToBind.apply((this instanceof fNOP&&oThis?this:oThis),aArgs.concat(Array.prototype.slice.call(arguments)));};fNOP.prototype=this.prototype;fBound.prototype=new fNOP();return fBound;};"use strict";p.isNormalClick=function(ev){return(ev.type!='click'||!(ev.which!=1||ev.shiftKey||ev.altKey||ev.metaKey||ev.ctrlKey));};p.KEY={'BACKSPACE':8,'TAB':9,'ENTER':13,'PAUSE':19,'CAPS':20,'ESC':27,'SPACE':32,'PAGE_UP':33,'PAGE_DOWN':34,'END':35,'HOME':36,'LEFT_ARROW':37,'UP_ARROW':38,'RIGHT_ARROW':39,'DOWN_ARROW':40,'INSERT':45,'DELETE':46,'0':48,'1':49,'2':50,'3':51,'4':52,'5':53,'6':54,'7':55,'8':56,'9':57,'A':65,'B':66,'C':67,'D':68,'E':69,'F':70,'G':71,'H':72,'I':73,'J':74,'K':75,'L':76,'M':77,'N':78,'O':79,'P':80,'Q':81,'R':82,'S':83,'T':84,'U':85,'V':86,'W':87,'X':88,'Y':89,'Z':90,'NUMPAD_0':96,'NUMPAD_1':97,'NUMPAD_2':98,'NUMPAD_3':99,'NUMPAD_4':100,'NUMPAD_5':101,'NUMPAD_6':102,'NUMPAD_7':103,'NUMPAD_8':104,'NUMPAD_9':105,'MULTIPLY':106,'ADD':107,'SUBTRACT':109,'DECIMAL':110,'DIVIDE':111,'F1':112,'F2':113,'F3':114,'F4':115,'F5':116,'F6':117,'F7':118,'F8':119,'F9':120,'F10':121,'F11':122,'F12':123,'SHIFT':16,'CTRL':17,'ALT':18,'PLUS':187,'COMMA':188,'MINUS':189,'PERIOD':190};(function($){var FastClick=function(el,callback){this.element=el;this.callback=callback;this.sx=this.sy=0;this.scroll=0;this.element.addEventListener('touchstart',this,false);};var noClickUntil=0;FastClick.prototype={handleEvent:function(e){switch(e.type){case'touchstart':this.onTouchStart(e);break;case'touchend':this.onTouchEnd(e);break;}},onTouchStart:function(e){this.start=Date.now();this.sx=e.touches[0].pageX-document.body.scrollTop;this.sy=e.touches[0].pageY-document.body.scrollLeft;this.scroll=document.body.scrollTop;this.element.addEventListener('touchend',this,false);noClickUntil=0;},onTouchEnd:function(e){this.element.removeEventListener('touchend',this,false);var cx=e.changedTouches[0].pageX-document.body.scrollTop,cy=e.changedTouches[0].pageY-document.body.scrollLeft;var dx=this.sx-cx,dy=this.sy-cy,scrolled=(this.scroll!=document.body.scrollTop),maxMove=32,maxTime=300;if(!scrolled&&Math.abs(dx)<maxMove&&Math.abs(dy)<maxMove&&(Date.now()-this.start)<maxTime){e.preventDefault();e.stopPropagation();noClickUntil=Date.now()+1000;return this.callback(e);}}};$.fn.fastclick=function(callback){if(p.mobile&&window.Touch){for(var i=0;i<this.length;i++){new FastClick(this[i],callback);}
this.click(function(e){if(noClickUntil<Date.now()){return callback(e);}
else{return false;}});}
else{this.click(callback);}
return this;};})(jQuery);(function(window){var host='pr0gramm.com';var useSubdomains=false;window.CONFIG={HOST:host,ABSOLUTE_PATH:'/',HEADER_HEIGHT:52,COOKIE:{NAME:'me',EXPIRE:3650},PATH:{IMAGES:(useSubdomains?'//img.'+host+'/':'/data/img/'),FULLSIZE:(useSubdomains?'//full.'+host+'/':'/data/fullsize/'),THUMBS:(useSubdomains?'//thumb.'+host+'/':'/data/thumb/')},LAYOUT:{THUMBS_PER_ROW:{MIN:3,MAX:8},THUMB:{WIDTH:128,HEIGHT:128,PADDING:4},WIDTH_CLASS:{TWO_SIDEBARS:{MAX_WIDTH:Infinity,SIDEBARS_WIDTH:400,MARGIN:24,CLASS_NAME:'two-sidebars'},ONE_SIDEBAR:{MAX_WIDTH:1440,SIDEBARS_WIDTH:200,MARGIN:20,CLASS_NAME:'one-sidebar'},NO_SIDEBAR:{MAX_WIDTH:780,SIDEBARS_WIDTH:0,MARGIN:0,CLASS_NAME:'no-sidebar'}},WIDTH_CLASS_FORCE_NO_SIDEBAR:{MAX_WIDTH:Infinity,SIDEBARS_WIDTH:0,MARGIN:24,CLASS_NAME:'wide-no-sidebar'}},ANALYTICS:{ENABLED:true,ACCOUNT:'UA-66268030-1'},COMMENTS_MAX_LEVELS:15,ITEM_SHOW_SCORE_AGE:3600,COMMENT_SHOW_SCORE_AGE:3600,AUTO_SYNC:{ENABLED:true,INTERVAL:60},SFW_FLAG:{SFW:{flag:1,name:'sfw'},NSFW:{flag:2,name:'nsfw'},NSFL:{flag:4,name:'nsfl'}},INBOX_EXPANDED_COUNT:5,TAGS_MIN_CONFIDENCE:0.2,TAGS_MAX_DISPLAY:4,LAST_LINEAR_COMMENTS_ITEM:128926,API:{ENDPOINT:'/api/',ALLOWED_UPLOAD_TYPES:['image/png','image/jpeg','image/gif','video/webm']},TAGS_INPUT_SETTINGS:{'defaultText':'Tags hinzufügen, mit Komma trennen','removeWithBackspace':true,'minChars':2,'maxChars':32},BOOKMARKLET:"javascript:"+"void((function(){"+"var%20e=document.createElement('script');"+"e.src='https://"+host+"/media/bookmarklet.min.js';"+"document.body.appendChild(e);"+"})());",ADS:{ACCOUNT:'61585078',REFRESH_INTERVAL:(120*1000)}};})(window);p.ads={UNIT:{},_gptLoaded:false,_definedSlots:{},_refreshInterval:0,requireGPT:function(){if(p.ads._gptLoaded){return;}
window.googletag=window.googletag||{};googletag.cmd=googletag.cmd||[];var gads=document.createElement('script');gads.async=true;gads.type='text/javascript';gads.src='//www.googletagservices.com/tag/js/gpt.js';document.getElementsByTagName('head')[0].appendChild(gads);googletag.cmd.push(function(){googletag.pubads().enableSingleRequest();googletag.pubads().disableInitialLoad();googletag.enableServices();});p.ads._refreshInterval=setInterval(p.ads.refresh.bind(p.ads),CONFIG.ADS.REFRESH_INTERVAL);p.ads._gptLoaded=true;},refresh:function(){if(!p.ads._gptLoaded){return;}
googletag.pubads().refresh();},fill:function(elements){if(!elements.length){return;}
p.ads.requireGPT();elements.each(function(){var $div=$(this);var id=$div.attr('id');var slotName='/'+CONFIG.ADS.ACCOUNT+'/'+
$div.data('slot')+
(p.user.flags==CONFIG.SFW_FLAG.SFW.flag?'':'-nsfw');if(!p.ads._definedSlots[slotName]){p.ads._definedSlots[slotName]=true;var dims=$div.data('size').split('x');var width=parseInt(dims[0])||0;var height=parseInt(dims[1])||0;googletag.cmd.push(function(){var slot=googletag.defineSlot(slotName,[width,height],id).addService(googletag.pubads());googletag.display(id);googletag.pubads().refresh([slot]);});}
else{googletag.cmd.push(function(){googletag.display(id);});}});}};(function(factory){if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(jQuery);}}(function($){var pluses=/\+/g;function raw(s){return s;}
...
return false;};}));p.LocalStore=p.Class.extend({storeName:'keyvalues',prefix:'default_',status:0,_db:null,_queue:[],init:function(name){this.prefix=name?(name+'_'):this.prefix;var that=this;var indexedDB=window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB;if(!indexedDB){that.status=p.LocalStore.STATUS.FAILED;that._execQueue();return;}
try{var request=indexedDB.open('LocalStore',2);request.onupgradeneeded=function(){try{request.result.deleteObjectStore(name);}catch(e){}
try{request.result.createObjectStore('keyvalues');}catch(e){}};request.onsuccess=function(){that.status=p.LocalStore.STATUS.OPEN;that._db=request.result;that._execQueue();};request.onerror=function(){that.status=p.LocalStore.STATUS.FAILED;that._execQueue();};}catch(err){that.status=p.LocalStore.STATUS.FAILED;that._execQueue();}},_execQueue:function(){var queue=this._queue;this._queue=[];for(var i=0;i<queue.length;i++){var q=queue[i];this._execRequest(q.mode,q.action,q.callback,q.params);}},_execRequest:function(mode,action,callback,params){if(this.status===p.LocalStore.STATUS.OPEN){try{var store=this._db.transaction(this.storeName,mode).objectStore(this.storeName);var req=store[action].apply(store,params);if(callback){req.onsuccess=function(){callback(null,req.result);};req.onerror=function(){callback(req.error,null);};}}catch(err){}}
...
else if(this.status===p.LocalStore.STATUS.FAILED&&callback){callback(this.status);}},getItem:function(key,callback){this._execRequest('readonly','get',callback,[this.prefix+key]);},setItem:function(key,value,callback){this._execRequest('readwrite','put',callback,[value,this.prefix+key]);},removeItem:function(key,callback){this._execRequest('readwrite','delete',callback,[this.prefix+key]);},clear:function(callback){this._execRequest('readwrite','clear',callback,[]);}});p.LocalStore.deleteDatabase=function(name){name=name||'LocalStore';var indexedDB=window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB;if(indexedDB){try{indexedDB.deleteDatabase(name);}catch(err){}}};p.LocalStore.STATUS={NOT_OPEN:0,FAILED:1,OPEN:2};p.VoteCache=p.Class.extend({votes:{items:{},tags:{},comments:{}},localStore:null,_storageRequestsRunning:0,_updateFromLogWaiting:null,_cachedLastSyncId:-1,setDatabaseName:function(name){name=(typeof name==='string')?name.replace(/[^\w]/,''):'default';this.localStore=new p.LocalStore(name);p.LocalStore.deleteDatabase('localforage');},clear:function(){this.votes.items={};this.votes.tags={};this.votes.comments={};this.localStore.clear();},getLastSyncData:function(callback){if(!this.localStore){return;}
var that=this;this.localStore.getItem('last_sync_id',function(error,id){if(error){return;}
var lastSyncId=(parseInt(id)||0);that.localStore.getItem('sync_token',function(error,token){if(that._cachedLastSyncId!==-1&&that._cachedLastSyncId!=lastSyncId){that.restoreFromLocal(function(){callback(lastSyncId,token);});}
else{callback(lastSyncId,token);}
that._cachedLastSyncId=lastSyncId;});});},createNewSyncToken:function(){var token=Math.random().toString(36).substr(2);this.localStore.setItem('sync_token',token);return token;},restoreFromLocal:function(callback){if(!this.localStore){return;}
this._restoreFromLocalCallback=callback;var that=this;for(var thing in this.votes){this._storageRequestsRunning+=3;this.localStore.getItem(thing+'_up',this.combineVotes.bind(this,thing,1));this.localStore.getItem(thing+'_down',this.combineVotes.bind(this,thing,-1));this.localStore.getItem(thing+'_fav',this.combineVotes.bind(this,thing,2));}},combineVotes:function(thing,dir,error,compacted){this._storageRequestsRunning--;var votes=this.votes[thing];if(compacted){for(var i=0;i<compacted.length;i++){votes[compacted[i]]=dir;}}
if(this._storageRequestsRunning==0){if(this._restoreFromLocalCallback){this._restoreFromLocalCallback();}
if(this._updateFromLogWaiting){var r=this._updateFromLogWaiting;this._updateFromLogInternal(r.log,r.lastId,r.callback);this._updateFromLogWaiting=null;}}},saveToLocal:function(){if(!this.localStore){return;}
for(var thing in this.votes){var splitted=this.splitVotes(this.votes[thing]);for(var action in splitted){var ids=splitted[action];if(ids.length){this.localStore.setItem(thing+'_'+action,ids);}}}},splitVotes:function(votes){var up=[],fav=[],down=[];for(var i in votes){var v=votes[i];var n=i|0;if(v===1){up.push(n);}
else if(v===2){fav.push(n);}
else{down.push(n);}}
return{up:up,fav:fav,down:down};},updateFromLog:function(log,lastId,callback){if(!this.localStore||!log.length){return;}
this._cachedLastSyncId=lastId;if(this._storageRequestsRunning!=0){this._updateFromLogWaiting={log:log,lastId:lastId,callback:callback};}
else{this._updateFromLogInternal(log,lastId,callback);}},_updateFromLogInternal:function(log,lastId,callback){this.localStore.setItem('last_sync_id',lastId);var VA=p.VoteCache.VOTE_ACTION;for(var i=0;i<log.length;i+=2){var thingId=log[i];var action=VA[log[i+1]];if(action&&action.vote===0){delete this.votes[action.thing][thingId];}
else if(action){this.votes[action.thing][thingId]=action.vote;}}
this.saveToLocal();callback();}});p.VoteCache.VOTE_ACTION=[{thing:'none',vote:0},{thing:'items',vote:-1},{thing:'items',vote:0},{thing:'items',vote:+1},{thing:'comments',vote:-1},{thing:'comments',vote:0},{thing:'comments',vote:+1},{thing:'tags',vote:-1},{thing:'tags',vote:0},{thing:'tags',vote:+1},{thing:'items',vote:+2}];p.User=p.Class.extend({id:null,paid:false,cookie:{},flags:1,flagsName:'sfw',name:'',admin:false,syncTimeout:null,showAds:true,inboxCount:0,syncToken:null,init:function(){this.voteCache=new p.VoteCache();this.loadCookie();if(this.id){this.voteCache.setDatabaseName(this.name);this.voteCache.restoreFromLocal(this.syncVotesOnCurrentView.bind(this));this.sync();}
else{p.merge(p.User.MARK,p.User.MARK_SFW_OVERRIDES);}},login:function(data,callback){this.externalCallback=callback;p.api.post('user.login',data,this.loginCallback.bind(this));},logout:function(){p.api.post('user.logout',{id:this.id});this.softLogout();},softLogout:function(){this.flags=CONFIG.SFW_FLAG.SFW.flag;this.cookie={};this.saveCookie();this.id=null;this.paid=0;if(this.syncTimeout){clearTimeout(this.syncTimeout);}},setFlags:function(flags){if(flags<1||flags>7){flags=CONFIG.SFW_FLAG.SFW.flag;}
...
else{this.unset('ads');if(this.paid){this.showAds=false;}}},loginCallback:function(response){if(response.success){this.loadCookie();this.voteCache.setDatabaseName(this.name);this.voteCache.restoreFromLocal(this.syncVotesOnCurrentView.bind(this));this.sync();}
if(this.externalCallback){this.externalCallback(response);}},get:function(key){return this.cookie[key]||null;},set:function(key,value){this.cookie[key]=value;this.saveCookie();},unset:function(key){delete this.cookie[key];this.saveCookie();},loadCookie:function(){var restored=JSON.parse($.cookie(CONFIG.COOKIE.NAME)||'{}');if(restored){this.cookie=p.merge(this.cookie,restored);this.id=this.cookie.id;this.flags=parseInt(this.cookie.fl)||1;if(this.flags<1||this.flags>7){this.flags=CONFIG.SFW_FLAG.SFW.flag;}
if(this.id){this.name=this.cookie.n;this.admin=!!this.cookie.a;this.paid=!!this.cookie.paid;}}
else{this.id=null;}
if(!this.id){this.flags=CONFIG.SFW_FLAG.SFW.flag;this.admin=false;this.paid=false;}
this.showAds=!!(!this.paid||this.cookie.ads);this.reloadFlagsName();},reloadFlagsName:function(){if(this.flags==(CONFIG.SFW_FLAG.SFW.flag|CONFIG.SFW_FLAG.NSFW.flag|CONFIG.SFW_FLAG.NSFL.flag)){this.flagsName='all';}
else{var names=[];for(var f in CONFIG.SFW_FLAG){if(this.flags&CONFIG.SFW_FLAG[f].flag){names.push(CONFIG.SFW_FLAG[f].name);}}
this.flagsName=names.length?names.join('+'):CONFIG.SFW_FLAG.SFW.name;}},saveCookie:function(){var cs=JSON.stringify(this.cookie);$.cookie(CONFIG.COOKIE.NAME,cs,{expires:CONFIG.COOKIE.EXPIRE,path:'/'});},sync:function(){this.voteCache.getLastSyncData((function(lastSyncId,globalSyncToken){if(!globalSyncToken||!this.syncToken||this.syncToken===globalSyncToken){this.syncToken=this.voteCache.createNewSyncToken();p.api.get('user.sync',{lastId:lastSyncId},this.syncCallback.bind(this),p.ServerAPI.SILENT);}
else{this.syncToken=globalSyncToken;this.voteCache.localStore.getItem('inbox_count',(function(error,count){this.setInboxLink(count||0);this.scheduleSync();}).bind(this));}}).bind(this));},syncCallback:function(response){this.voteCache.localStore.setItem('inbox_count',response.inboxCount);this.setInboxLink(response.inboxCount);this.loadCookie();this.voteCache.updateFromLog(response.log,response.lastId,this.syncVotesOnCurrentView.bind(this));this.scheduleSync();},scheduleSync:function(){if(this.id&&CONFIG.AUTO_SYNC.ENABLED){this.syncTimeout=setTimeout(this.sync.bind(this),CONFIG.AUTO_SYNC.INTERVAL*1000);}},restoreVotes:function(){this.voteCache.clear();p.api.get('user.sync',{lastId:0},this.syncCallback.bind(this),p.ServerAPI.SILENT);},setInboxLink:function(count){this.inboxCount=count;var empty=(this.inboxCount==0);document.title=(empty?'':'['+count+'] ')+document.title.replace(/^\[\d+\]\s*/,'');$('#inboxLink').toggleClass('empty',empty);$('#inboxLink').attr('href',empty?'/inbox/all':'/inbox/unread');$('#inboxCount').text(this.inboxCount);},syncVotesOnCurrentView:function(){if(p.currentView&&p.currentView.syncVotes){p.currentView.syncVotes(this.voteCache.votes);}}});p.User.MARK={'0':'Schwuchtel','1':'Neuschwuchtel','2':'Altschwuchtel','3':'Admin','4':'Gesperrt','5':'Moderator','6':'Fliesentischbesitzer','7':'Lebende Legende','8':'pr0wichtler','9':'Edler Spender',};p.User.MARK_SFW_OVERRIDES={'1':'Neu','2':'Alt'};p.ServerAPI=p.Class.extend({path:'/',init:function(path){this.path=path||'/';},post:function(endpoint,data,callback,errback){this._ajax('POST',endpoint,data,callback,errback);},get:function(endpoint,data,callback,errback){this._ajax('GET',endpoint,data,callback,errback);},_ajax:function(method,endpoint,data,callback,errback){if(data instanceof HTMLFormElement||((data instanceof $)&&data.is('form'))){data=$(data).serialize();if(data.length>0&&p.user.id&&method=='POST'){data=data+'&_nonce='+p.user.id.substr(0,16);}}
else if(p.user.id&&method=='POST'){if(typeof(data)=='object'){data._nonce=p.user.id.substr(0,16);}
else if(typeof(data)=='string'){data=data+'&_nonce='+p.user.id.substr(0,16);}}
var url=this.path+endpoint.replace(/\./g,'/');if(this.onerror&&errback!==p.ServerAPI.SILENT){if(errback){var onerror=this.onerror;var paramErrback=errback;errback=function(response){onerror(response);paramErrback(response);}}
else{errback=this.onerror;}}
$.ajax({type:method,url:url,success:callback,error:errback,dataType:'json',data:data});}});p.ServerAPI.SILENT=function(){};p.Hotkeys=p.Class.extend({$indicator:null,init:function(element){$(element).keydown(this.keydown.bind(this));},keydown:function(ev){if(ev.altKey||ev.ctrlKey||ev.metaKey){return;}
if(ev.target.type=='text'||ev.target.type=='textarea'||ev.target.type=='email'||ev.target.type=='password'||ev.target.className=='button'){if(ev.keyCode==p.KEY.ESC){ev.target.blur();}
return;}
if(this.handleKey(ev.keyCode)){ev.preventDefault();return false;}
return true;},handleKey:function(code){switch(code){case p.KEY.LEFT_ARROW:case p.KEY.A:$('.stream-prev:visible').click();return true;case p.KEY.RIGHT_ARROW:case p.KEY.D:$('.stream-next:visible').click();return true;case p.KEY.ESC:$('.item-image:visible').click();return true;case p.KEY.K:$('#search-form-inline:visible input.q').focus();return true;case p.KEY.C:$('.comment:visible').focus();return true;case p.KEY.T:$('.add-tags-link:visible').click();return true;case p.KEY.F:case p.KEY.MULTIPLY:if($('.vote-fav').click().length){this.showKeyIndicator($('.vote-fav.faved').length?'*':'+',true);}
...
return true;case p.KEY.N:if(p.user.admin){var itemId=$('.tag-form input[name=itemId]').val();if(itemId){this.showKeyIndicator('nsfw',false);p.api.post('tags.add',{tags:'nsfw',itemId:itemId});}}
return true;}
...
this.$indicator.stop().css('opacity',1).toggleClass('pict',isPict).text(text).show().delay(500).fadeOut('fast');}});"use strict";p.voteClass=function(vote){return vote?(vote>0?' voted-up':' voted-down'):'';};p.favClass=function(vote){return vote==2?' faved':'';};p.adjustVote=function(thing,vote,cached){if(!thing){return;}
if(thing.vote<0){thing.down--;}
else if(thing.vote>0){thing.up--;}
if(vote>0){thing.up++;}
else if(vote<0){thing.down++;}
thing.vote=vote;if(cached&&thing.id){cached[thing.id]=vote;}};p.View={};p.View.Base=p.Class.extend({visible:false,needsRendering:false,aborted:false,compiledTemplate:null,template:null,children:[],data:{},dataReady:true,parentView:null,requiresLogin:false,loginUrl:'overlay',$container:null,init:function(container,parent){if(this.requiresLogin&&!p.user.id){this.aborted=true;if(this.loginUrl=='overlay'){p.mainView.showLogin();}
...
this.compiledTemplate=p._compiledTemplates[this.classId];if(!this.compiledTemplate){this.compiledTemplate=p.compileTemplate(this.template);p._compiledTemplates[this.classId]=this.compiledTemplate;}},remove:function(){this.visible=false;if(!this.parentView){return;}
this.parentView.children.erase(this);},resize:function(){for(var i=0;i<this.children.length;i++){this.children[i].resize();}},load:function(){return true;},render:function(){if(!this.visible){return;}
if(!this.$container){this.$container=$(this.compiledTemplate(this.data));}
...
return this.showOverlay(p.View.Overlay.Error,response.responseJSON);}},setUserStatus:function(isUser){$('#page').toggleClass('status-user',isUser).toggleClass('status-guest',!isUser);},search:function(q){var m=null;if(m=q.match(/(\w+):(likes|uploads)/)){p.navigateTo('user/'+m[1]+'/'+m[2]);}
else{var tab=$('#tab-top').hasClass('active')?'top':'new';var path=q?('/'+encodeURIComponent(q)):'';path=path.match(/^\/\d+$/)?path.replace('/','/+'):path;p.navigateTo(tab+path);}
$('#search-form-bar').hide();},show:function(params){this.data.currentFilterName=p.user.flagsName;this.parent(params);this.setPageCSS(this.thumbsPerRow);this.setUserStatus(!!p.user.id);$('#login-link').click(this.showLogin.bind(this));this.overlay=$('#overlay');this.overlay.click(this.hideOverlay.bind(this));$('#filter-link').fastclick(function(){$('.filter-setting').each(function(){var el=$(this);el.toggleClass('active',!!(p.user.flags&parseInt(el.data('flag'))));});$('#filter-menu').fadeToggle(100);return false;});$('.filter-setting').fastclick(function(ev){$(ev.currentTarget).toggleClass('active').blur();return false;});$('#filter-save').fastclick(function(){var flags=0;$('.filter-setting.active').each(function(){flags+=parseInt($(this).data('flag'));});p.user.setFlags(flags);p.reload();$('#filter-menu').fadeOut(100);return false;});var that=this;$('form.search-form').submit(function(ev){that.search($(ev.target).find('input.q').val());ev.preventDefault();return false;});$('#search-button').fastclick(function(){var searchBar=$('#search-form-bar');if(searchBar.is(':visible')){searchBar.find('input.q').blur();searchBar.hide();}
else{searchBar.show();searchBar.find('input.q').focus();}});p.ads.fill(this.$container.find('.gpt:visible'));},setTab:function(tab){if(tab=='new'){$('#tab-new').addClass('active');$('#tab-top').removeClass('active');$('input.q').attr('placeholder','Alles durchsuchen');$('#search-form-bar .search-submit').val('Alles durchsuchen');}else if(tab=='top'){$('#tab-top').addClass('active');$('#tab-new').removeClass('active');$('input.q').attr('placeholder','Beliebt durchsuchen');$('#search-form-bar .search-submit').val('Beliebt durchsuchen');}
else{$('#tab-new, #tab-top').removeClass('active');}
$('.head-tab').removeClass('active');$('#tab-'+tab).addClass('active');var qs=tab=='new'?'Alles durchsuchen':'Beliebt durchsuchen';$('input.q').attr('placeholder',qs);$('#search-form-bar .search-submit').val(qs);},requireLogin:function(){if(!p.user.id){this.showOverlay(p.View.Overlay.Login);return false;}
...
var overlayBox=$('#overlay-box');overlayBox.attr('class',overlayClass.prototype.name||'overlay-generic');this.overlayView=new(overlayClass)(overlayBox,this,param);this.overlayView.show();if(p.mobile){$(document).scrollTop(0);this.overlay.addClass('mobile');this.overlay.css('height',$('#main-view').height());}
this.overlay.fadeIn('fast');return false;},hideOverlay:function(ev){if(ev.target!=this.overlay[0]){return;}
return this.closeOverlay();},closeOverlay:function(){if(!this.overlayView){return false;}
$('#page').removeClass('in-background');this.overlay.fadeOut('fast');this.overlayView.remove();this.overlayView=null;return false;}});"use strict";p.Stream=p.Class.extend({reached:{start:false,end:false},_oldestId:Number.POSITIVE_INFINITY,_newestId:0,items:{},init:function(options){this.options=options||{};},loadInfo:function(id,callback){var item=this.items[id];if(!item){return;}
if(item.infoTime){if(callback){callback(item);}
return;}
if(item.loadQueue){if(callback){item.loadQueue.push(callback);}
return;}
item.loadQueue=callback?[callback]:[];var that=this;p.api.get('items.info',{itemId:id},function(response){that._processInfo(item,response);});},getLoadedItems:function(){var itemsArray=[];for(var id in this.items){itemsArray.push(this.items[id]);}
...
that.data('_highlightTimeout',setTimeout(step,wait||200));};})(jQuery);p.View.Overlay=p.View.Overlay||{};p.View.Overlay.DeleteComment=p.View.Base.extend({name:'overlay-delete-comment',template:'<div class="overlay-content"> <h2>Kommentar löschen</h2> </div> <div class="overlay-content"> <form id="comment-delete-form"> <div class="form-row"> <input type="text" class="wide" name="reason" placeholder="Grund"/> </div> <div class="form-row"> <input type="hidden" name="id" value="{commentId}"/> <input type="submit" value="Durch [gelöscht] ersetzen" id="comment-delete-soft"/> <input type="submit" value="Hart Löschen (inkl. aller antworten)" id="comment-delete-hard"/> </div> </form> </div> ',data:{commentId:0},init:function(container,parent,$div){var $comment=$div.parent();this.data.commentId=$comment.find('.permalink').attr('href').split(':comment')[1];this.parent(container,parent);},render:function(){this.parent();this.$container.find('form').submit(this.submit.bind(this));this.focus('input[name=reason]');},submit:function(ev){var $button=this.$container.find('input[type=submit]:focus');var action=($button.attr('id')=='comment-delete-hard')?'comments.delete':'comments.softDelete';p.api.post(action,ev.target,this.posted.bind(this));this.renderLoading();return false;},posted:function(response){p.mainView.closeOverlay();}});p.View.Stream=p.View.Stream||{};p.View.Stream.Comments=p.View.Base.extend({template:' <div class="comments-head"> <span class="pict">c</span> {"Kommentar".inflect(commentCount)} </div> <?js if(p.user.showAds) {?> <div class="comments-large-rectangle gpt" id="gpt-rectangle-comments" data-size="336x280" data-slot="pr0gramm-rectangle"> </div> <?js } ?> <form class="comment-form" method="post"> <textarea class="comment" name="comment" required placeholder="Kommentar schreiben…"></textarea> <input type="hidden" name="parentId" value="0"/> <input type="hidden" name="itemId" value="{params.id}"/> <div> <input type="submit" value="Abschicken"/> <input type="button" value="Abbrechen" class="cancel"/> </div> </form> <form class="comment-edit-form" method="post"> <textarea class="comment" required name="comment"></textarea> <input type="hidden" name="commentId" value="0"/> <div> <input type="submit" value="Abschicken"/> <input type="button" value="Abbrechen" class="cancel"/> </div> </form> <div class="comments"> <?js var recurseComments = function( comments, level ) { ?> <div class="comment-box"> <?js for( var i = 0; i < comments.length; i++ ) { var c = comments[i]; ?> <div class="comment{p.voteClass(c.vote)}<?js if(c.name == op){?> comment-op<?js}?>" id="comment{c.id}"> <div class="comment-vote"> <span class="pict vote-up">+</span> <span class="pict vote-down">-</span> </div> <div class="comment-content"> {c.content.format()} </div> <div class="comment-foot"> <a href="#user/{c.name}" class="user um{c.mark}">{c.name}</a> <?js if( c.showScore || p.user.admin ) {?> <span class="score" title="{c.up} up, {c.down} down">{"Punkt".inflect(c.score)}</span> <?js } else { ?> <span class="score-hidden" title="Score noch unsichtbar">●●●</span> <?js } ?> <a href="#{tab}/{itemId}:comment{c.id}" class="time permalink" title="{c.createdReadable}">{c.createdRelative}</a> <?js if( level < CONFIG.COMMENTS_MAX_LEVELS && !linearComments ) {?> <a href="#{tab}/{itemId}:comment{c.id}" class="comment-reply-link action"><span class="pict">r</span> antworten</a> <?js } ?> <?js if( /*c.user == p.user.name ||*/ p.user.admin ) {?> [ <span class="comment-delete action">del</span> / <a href="#{tab}/{itemId}:comment{c.id}" class="comment-edit-link action">edit</a> ] <?js } ?> </div> </div> <?js if( c.children.length ) { recurseComments(c.children, level+1); } ?> <?js } ?> </div> <?js }; ?> <?js recurseComments(comments, 1); ?> </div> ',init:function(container,parent){this.stream=parent.stream;this.parent(container,parent);},load:function(){this.stream.loadInfo(this.data.params.id,this.loaded.bind(this));return false;},syncVotes:function(votes){if(!this.data.comments){return;}
var commentVotes=votes.comments;for(var id in this.commentMap){var newVote=commentVotes[id];var prevVote=this.commentMap[id].vote;if(newVote!=prevVote){var node=$('#comment'+id);this.commentMap[id].vote=newVote||0;node.removeClass('voted-up voted-down');if(newVote){node.addClass(newVote===1?'voted-up':'voted-down');}}}},loaded:function(item){item.id=(item.id||this.data.itemId);this.data.linearComments=(item.id<=CONFIG.LAST_LINEAR_COMMENTS_ITEM);if(item.commentId){p.user.voteCache.votes.comments[item.commentId]=1;this.data.params.comment='comment'+item.commentId;}
this.data.comments=this.prepareComments(item.comments);this.stream.items[this.data.params.id].comments=item.comments;this.data.commentCount=item.comments.length;this.data.tab=this.parentView.parentView.tab||'new';this.data.itemId=item.id;this.data.op=item.user;this.render();},prepareComments:function(comments){if(this.data.linearComments){comments.sort(p.View.Stream.Comments.SortTime);}
else{comments.sort(p.View.Stream.Comments.SortConfidenceTime);}
var commentMap={'0':{children:[]}};var commentVotes=p.user.voteCache.votes.comments;for(var i=0;i<comments.length;i++){var comment=comments[i];var created=new Date(comment.created*1000);comment.children=[];comment.vote=commentVotes[comment.id]||0;comment.createdRelative=created.relativeTime(true);comment.createdReadable=created.readableTime();comment.score=comment.up-comment.down;comment.showScore=(comment.created+CONFIG.COMMENT_SHOW_SCORE_AGE<Date.now()/1000);commentMap[comment.id]=comment;}
...
this.commentMap=commentMap;return commentMap['0'].children;},render:function(){this.parent();this.$commentForm=this.$container.find('.comment-form');this.$commentForm.submit(this.submitComment.bind(this));this.$commentForm.find('input.cancel').hide().click(this.cancelReply.bind(this));this.$commentEditForm=this.$container.find('.comment-edit-form');this.$commentEditForm.submit(this.submitCommentEdit.bind(this));this.$commentEditForm.find('input.cancel').click(this.cancelEdit.bind(this));this.$container.find('.comment-reply-link').fastclick(this.showReplyForm.bind(this));this.$container.find('.comment-edit-link').fastclick(this.showEditForm.bind(this));var that=this;this.$container.find('.vote-up').fastclick(function(ev){return that.vote(ev,1);});this.$container.find('.vote-down').fastclick(function(ev){return that.vote(ev,-1);});this.$container.find('.comment-delete').fastclick(function(){p.mainView.showOverlay(p.View.Overlay.DeleteComment,$(this));});if(this.data.params.comment){if(this.parentView.heightKnown){this.focusComment(that.data.params.comment);}
this.data.params.comment=null;}
p.ads.fill(this.$container.find('.gpt:visible'));},focusComment:function(comment){var target=this.$container.find('#'+comment);if(target.length){var jumpPos=target.offset().top-CONFIG.HEADER_HEIGHT-80;target.highlight(180,180,180,1);$(document).scrollTop(jumpPos);}},vote:function(ev,vote){if(!p.mainView.requireLogin()){return false;}
...
p.api.post('tags.vote',{id:tagId,vote:absoluteVote});return false;}});p.View.Stream.Tags.SortConfidence=function(a,b){return b.confidence-a.confidence;};p.View.Overlay=p.View.Overlay||{};p.View.Overlay.DeleteItem=p.View.Base.extend({name:'overlay-delete-item',template:'<div class="overlay-content"> <h2>Bild löschen</h2> </div> <div class="overlay-content"> <form id="image-delete-form"> <div class="form-row"> <label class="checkbox-label"> <input name="reason" type="radio" value="custom" checked id="custom"/> </label> <input type="text" class="text-line" name="customReason" placeholder="Grund"/> <label> <input name="reason" type="radio" value="Regel #1 - Bild unzureichend getagged (nsfw/nsfl)"/> Regel #1 - Bild unzureichend getagged (nsfw/nsfl) </label> <label> <input name="reason" type="radio" value="Regel #2 - Gore/Porn/Suggestive Bilder mit Minderjährigen"/> Regel #2 - Gore/Porn/Suggestive Bilder mit Minderjährigen </label> <label> <input name="reason" type="radio" value="Regel #3 - Tierporn"/> Regel #3 - Tierporn </label> <label> <input name="reason" type="radio" value="Regel #4 - Stumpfer Rassismus/Nazi-Nostalgie"/> Regel #4 - Stumpfer Rassismus/Nazi-Nostalgie </label> <label> <input name="reason" type="radio" value="Regel #5 - Werbung/Spam"/> Regel #5 - Werbung/Spam </label> <label> <input name="reason" type="radio" value="Regel #6 - Infos zu Privatpersonen"/> Regel #6 - Infos zu Privatpersonen </label> <label> <input name="reason" type="radio" value="Regel #7 - Bildqualität"/> Regel #7 - Bildqualität </label> <label> <input name="reason" type="radio" value="Regel #8 - Ähnliche Bilder in Reihe"/> Regel #8 - Ähnliche Bilder in Reihe </label> <label> <input name="reason" type="radio" value="Regel #12 - Warez/Logins zu Pay Sites"/> Regel #12 - Warez/Logins zu Pay Sites </label> <label> <input name="reason" type="radio" value="Repost"/> Repost </label> <label> <input name="reason" type="radio" value="Auf Anfrage"/> Auf Anfrage </label> </div> <div class="form-row divider"> <input type="checkbox" name="notifyUser" checked="checked" id="delete-comment-notify"/> <label class="checkbox-label" for="delete-comment-notify">Benutzer benachrichtigen</label> </div> <div class="form-row"> <input type="checkbox" name="banUser" id="delete-ban-user"/> <label class="checkbox-label" for="delete-ban-user">Benutzer Sperren für</label> <input type="text" name="days" class="number" value="1"/> Tage (0 = für immer) </div> <div class="form-row"> </div> <div class="form-row"> <input type="hidden" name="id" value="{itemId}"/> <input type="submit" value="Löschen" id="image-delete"/> </div> </form> </div> ',data:{itemId:0},init:function(container,parent,$el){this.data.itemId=$el.data('id');this.parent(container,parent);},render:function(){this.parent();this.$container.find('form').submit(this.submit.bind(this));var customRadionButton=this.$container.find('input[name=reason][value=custom]');this.$container.find('input[name=customReason]').bind('focus onchange',function(){customRadionButton.prop('checked',true);});this.focus('input[name=customReason]');},submit:function(ev){p.api.post('items.delete',ev.target,this.posted.bind(this));this.renderLoading();return false;},posted:function(response){p.mainView.closeOverlay();}});p.Video={canPlayWebM:true,ready:false,onready:null};try{var probe=document.createElement('video');if(p.mobile||!probe||!probe.canPlayType||!probe.canPlayType('video/webm; codecs="vp8"')){p.Video.canPlayWebM=false;$.getScript('/frontend/lib/jsmpeg.min.js',function(data,textStatus,jqxhr){p.Video.ready=true;if(p.Video.onready){var func=p.Video.onready;p.Video.onready=null;func();}});}}catch(error){};p.View.Stream=p.View.Stream||{};p.View.Stream.Item=p.View.Base.extend({template:'<div class="item-pointer"> </div> <div class="item-container-content"> <div class="item-image-wrapper"> <?js if( item.video ) { ?> <?js if( canPlayWebM ) { ?> <video class="item-image" src="{item.image}" type="video/webm" autoplay loop></video> <div class="video-position-bar"> <div class="video-position-bar-background"> <div class="video-position"></div> </div> </div> <?js } else { ?> <canvas class="item-image"></canvas> <?js } ?> <?js } else { ?> <img class="item-image" src="{item.image}"/> <?js if(item.fullsize) { ?> <a href="{item.fullsize}" target="_blank" class="item-fullsize-link">+</a> <?js } ?> <?js } ?> <?js if( p.user.showAds ) { ?> <div class="stream-prev" title="Vorwärts"><span class="stream-prev-icon"></span></div> <div class="stream-next" title="Zurück"><span class="stream-next-icon"></span></div> <?js } else { ?> <div class="stream-prev arrow pict" title="Vorwärts">&lt;</div> <div class="stream-next arrow pict" title="Zurück">&gt;</div> <?js } ?> </div> <div class="item-info"> <div class="item-vote{p.voteClass(item.vote)}"> <span class="pict vote-up">+</span> <span class="pict vote-down">-</span> <?js if( item.showScore || p.user.admin ) {?> <span class="score<?js if(!item.showScore){?> score-young<?js}?>" title="{item.up} up, {item.down} down"><?js print(item.up - item.down)?></span> <?js } else { ?> <span class="score-hidden" title="Score noch unsichtbar">●●●</span> <?js } ?> </div> <?js if( item.user != p.user.name ) {?> <span class="pict vote-fav{p.favClass(item.vote)}">*</span> <?js } ?> <div class="item-details"> <a class="time" title="{item.time.readableTime()}" href="/new/{item.id}">{item.time.relativeTime(true)}</a> <span class="time">von</span> <a href="#user/{item.user}" class="user um{item.mark}">{item.user}</a> <span class="item-source"> <?js if( item.source ) {?> <span class="pict">s</span>&nbsp;<a href="{{item.source}}" target="_blank">{{item.source.hostName()}}</a> <?js } else { ?> <span class="pict">s</span>upload</span> <?js } ?> </span> <?js if( !item.video ) {?> <span class="item-google-search"> <span class="pict">g</span>&nbsp;<a href="https://www.google.com/searchbyimage?hl=en&amp;safe=off&amp;site=search&amp;image_url=http:{item.image}" target="_blank"> Bild googeln </a> </span> <?js } ?> <?js if( p.user.admin ) { ?> [<span class="action" id="item-delete" data-id="{item.id}">del</span>] [<a href="/new/phash.{item.id}.12">phash</a>] <span class="flags flags-{item.flags}">{p.Stream.FLAG_NAME[item.flags]}</span> <?js } ?> </div> <div class="item-tags"></div> </div> <?js if(p.user.showAds) {?> <div class="divider-full-banner gpt" id="gpt-divider-banner" data-size="468x60" data-slot="pr0gramm-banner"> </div> <div class="divider-large-rectangle gpt" id="gpt-divider-rectangle" data-size="336x280" data-slot="pr0gramm-rectangle"> </div> <?js } ?> <div class="item-comments"></div> </div> ',swipeDistance:96,navTop:-1,heightKnown:false,init:function(container,parent){this.stream=parent.stream;this.parent(container,parent);},show:function(rowIndex,itemData,defaultHeight,jumpToComment){if(this.removed){return;}
this.data.canPlayWebM=p.Video.canPlayWebM;this.data.item=itemData;this.data.item.video=!!this.data.item.image.match(/\.webm$/);this.data.item.time=new Date(itemData.created*1000);this.data.item.showScore=(itemData.created+CONFIG.ITEM_SHOW_SCORE_AGE<Date.now()/1000);this.parent();this.$image=this.$container.find('.item-image');if(this.data.item.video&&!p.Video.canPlayWebM){if(!p.Video.ready){p.Video.onready=this.show.bind(this,rowIndex,itemData,defaultHeight,jumpToComment);return;}
var mpeg=this.data.item.image.replace(/\.webm$/,'.mpg');this.jsmpeg=new jsmpeg(mpeg,{canvas:this.$image[0],autoplay:true,loop:true});}
var T=CONFIG.LAYOUT.THUMB;var offset=rowIndex*(T.WIDTH+T.PADDING)+T.WIDTH/2-T.PADDING-4;this.$container.find('.item-pointer').css('left',offset);this.$image.fastclick(this.parentView.hideItem.bind(this.parentView));if(!this.$image.height()||(this.data.item.video&&p.Video.canPlayWebM)){if(defaultHeight){this.$container.find('.item-image-wrapper').css('height',defaultHeight);}
var imageLoadedBound=this.imageLoaded.bind(this);if(this.data.item.video){this.video=this.$image[0];this.video.addEventListener('loadedmetadata',imageLoadedBound,false);}
else{this.$image.load(imageLoadedBound);this.pollForHeightInterval=setInterval(imageLoadedBound,10);}}
else{this.heightKnown=true;}
var that=this;this.$itemVote=this.$container.find('.item-vote');this.$itemVote.find('.vote-up').fastclick(function(ev){return that.vote(ev,1);});this.$itemVote.find('.vote-down').fastclick(function(ev){return that.vote(ev,-1);});this.$itemFav=this.$container.find('.vote-fav')
this.$itemFav.fastclick(function(ev){return that.fav(ev);});this.tagView=new p.View.Stream.Tags(this.$container.find('.item-tags'),this);this.tagView.show({id:itemData.id});this.commentView=new p.View.Stream.Comments(this.$container.find('.item-comments'),this);this.commentView.show({id:itemData.id,comment:jumpToComment});this.jumpToComment=jumpToComment;if(p.mobile){this.setupSwipe();}
else{this.$streamPrev=this.$container.find('.stream-prev');this.$streamNext=this.$container.find('.stream-next');this.$streamPrev.show().fastclick(this.parentView.prev.bind(this.parentView));this.$streamNext.show().fastclick(this.parentView.next.bind(this.parentView));this.onScrollBound=this.onScroll.bind(this);$(window).bind('scroll',this.onScrollBound);this.onScroll();}
this.$container.find('#item-delete').fastclick(function(){p.mainView.showOverlay(p.View.Overlay.DeleteItem,$(this));});p.ads.fill(this.$container.find('.gpt:visible'));},onScroll:function(){if(!this.heightKnown){return;}
var ih=this.$image.height();var h=ih/2-32;var p=($(window).scrollTop()-this.$container.offset().top).limit(32,ih-32-96)+96;if(p==this.navTop){return;}
this.navTop=p;this.$streamPrev.css('padding-top',p);this.$streamNext.css('padding-top',p);},updateVideoPosition:function(){if(this.video&&this.video.duration){var through=(this.video.currentTime/this.video.duration);if(through<this.videoThrough){this.videoThrough=through;}
this.videoThrough=0.9*this.videoThrough+0.1*through;this.$videoPosition.css('width',(this.videoThrough*100).round(3)+'%')}},jumpToPosition:function(ev){if(this.video&&this.video.duration){this.videoThrough=(ev.pageX-this.$videoPositionBar.offset().left)/this.$videoPositionBar.width();this.video.currentTime=this.video.duration*this.videoThrough;}},remove:function(){this.parent();this.removed=true;if(this.jsmpeg){this.jsmpeg.stop();this.jsmpeg=null;}
if(this.updateVideoPositionInterval){clearInterval(this.updateVideoPositionInterval);}
if(this.onScrollBound){$(window).unbind('scroll',this.onScrollBound);}},syncVotes:function(votes){if(!this.data.item){return;}
var vote=votes.items[this.data.item.id];if(vote&&this.$itemVote){this.$itemVote.removeClass('voted-up voted-down');this.$itemVote.addClass(vote>0?'voted-up':'voted-down');this.$itemFav.toggleClass('faved',(vote==2));}
if(this.commentView){this.commentView.syncVotes(votes);}
if(this.tagView){this.tagView.syncVotes(votes);}},imageLoaded:function(ev){if(!ev&&!this.$image[0].height||this.heightKnown){return;}
this.heightKnown=true;this.$container.find('.item-image-wrapper').css('height','auto');if(this.pollForHeightInterval){clearInterval(this.pollForHeightInterval);this.pollForHeightInterval=0;}
if(this.video){var width=Math.min(this.$image[0].videoWidth,this.$container.width());this.$container.find('.video-position-bar').css({width:width}).show();this.$videoPositionBar=this.$container.find('.video-position-bar');this.$videoPosition=this.$container.find('.video-position');this.videoThrough=0;this.updateVideoPositionInterval=setInterval(this.updateVideoPosition.bind(this),16);this.$videoPositionBar.fastclick(this.jumpToPosition.bind(this));}
if(this.commentView&&this.jumpToComment){this.commentView.focusComment(this.jumpToComment);this.jumpToComment=null;}
if(this.onScrollBound){this.onScroll();}},fav:function(ev){if(!p.mainView.requireLogin()){return false;}
var addFav=!this.$itemFav.hasClass('faved');var absoluteVote=addFav?2:1;this.$itemFav.toggleClass('faved',addFav);this.$itemVote.addClass('voted-up');this.$itemVote.removeClass('voted-down');var cachedItem=this.stream.items[this.data.item.id];p.adjustVote(cachedItem,absoluteVote,p.user.voteCache.votes.items);var $score=this.$itemVote.find('.score');$score.text(cachedItem.up-cachedItem.down);p.api.post('items.vote',{id:this.data.item.id,vote:absoluteVote});return false;},vote:function(ev,vote){if(!p.mainView.requireLogin()){return false;}
...
this.$itemFav.removeClass('faved');var cachedItem=this.stream.items[this.data.item.id];p.adjustVote(cachedItem,absoluteVote,p.user.voteCache.votes.items);var $score=this.$itemVote.find('.score');$score.text(cachedItem.up-cachedItem.down);p.api.post('items.vote',{id:this.data.item.id,vote:absoluteVote});return false;},lastTouch:{x:0,y:0},avgTouchDelta:{x:0,y:0},touchOffset:{x:0,y:0},setupSwipe:function(){this.$image.bind('touchstart',this.initSwipeImage.bind(this));this.$image.bind('touchmove',this.swipeImage.bind(this));this.$image.bind('touchend',this.endSwipeImage.bind(this));},initSwipeImage:function(ev){var touch=ev.originalEvent.touches[0];this.lastTouch.x=touch.clientX;this.lastTouch.y=touch.clientY;this.avgTouchDelta.x=0;this.avgTouchDelta.y=0;this.touchOffset.x=0;this.touchOffset.y=0;},endSwipeImage:function(ev){if(this.touchOffset.x>this.swipeDistance){this.parentView.prev();}
else if(this.touchOffset.x<-this.swipeDistance){this.parentView.next();}
else{this.touchOffset.x=0;this.touchOffset.y=0;this.$image.css('margin-left',this.touchOffset.x);}},swipeImage:function(ev){if(ev.originalEvent.touches.length>1){return;}
...
this.$image.css('margin-left',this.touchOffset.x);}});p.View.Stream=p.View.Stream||{};p.View.Stream.Main=p.View.Base.extend({baseURL:'',template:' <div id="stream"></div> <div class="clear"></div> ',SCROLL:{NONE:1,THUMB:2,FULL:4},scrollPosition:0,loadInProgress:false,loadingTargetDistance:2048,hasItems:false,prependNextSection:false,jumpToItem:null,jumpToComment:null,currentItemId:null,_scrolledToFullView:false,itemsPerRow:6,rowMargin:4,prefetched:[],init:function(container,parent){this.handleThumbLinkBound=this.handleThumbLink.bind(this);this.parent(container,parent);this.loadedBound=this.loaded.bind(this);this.scrollBound=this.onscroll.bind(this);$(window).scroll(this.scrollBound);this.$itemContainerTemplate=$('<div class="item-container"/>');},fragmentChange:function(fragment){if(this.currentItemSubview&&this.currentItemSubview.commentView&&fragment){this.currentItemSubview.commentView.focusComment(fragment);}},show:function(params){if(!params.userName){this.tab=params.tab||'top';}
else{this.tab=null;}
if(params.tab==='stalk'&&!p.user.paid){p.navigateTo('pr0mium');return;}
p.mainView.setTab(this.tab);var q=params.userName?params.userName+':'+params.userTab:params.tags||'';$('input[name=q]').val(q.replace('+',''));var newBaseURL=(p._hasPushState?'/':'#')+
(params.userName?('user/'+params.userName+'/'+params.userTab+'/'):(this.tab+'/'+(params.tags?encodeURIComponent(params.tags)+'/':'')));if(newBaseURL==this.baseURL){this.data.params=params;if(this.data.params.itemId){var $target=$('#item-'+this.data.params.itemId);if($target.length){$(document).scrollTop($target.offset().top-CONFIG.HEADER_HEIGHT);this.showItem($target,this.SCROLL.THUMB);return;}}
...
else if(params.userName&&params.userTab==='likes'){options.likes=params.userName;if(params.userName===p.user.name){options.self=true;}}
this.stream=new p.Stream(options);this.baseURL=newBaseURL;this.hasItems=false;this.parent(params);this.$streamContainer=this.$container.find('#stream');this.$streamContainer.append(p.View.Base.LoadingAnimHTML);this.$itemContainer=null;},resize:function(){this.jumpToItem=this.currentItemId;this.jumpToComment=p.getFragment();this.$streamContainer.html('');this.loaded(this.stream.getLoadedItems(),p.Stream.POSITION.APPEND);this.parent();},hide:function(){$(window).unbind('scroll',this.scrollBound);this.parent();},syncVotes:function(votes){if(this.stream){this.stream.syncVotes(votes);}
...
return true;},prepareThumbsForInsertion:function(html){var thumbs=$(html);thumbs.find('a').fastclick(this.handleThumbLinkBound);return thumbs;},handleThumbLink:function(ev){if(!p.isNormalClick(ev)){return true;}
ev.preventDefault();this.showItem($(ev.currentTarget),this.SCROLL.NONE);if(!this._wasHidden){this.handleHashLink(ev);}
this._wasHidden=false;return false;},loaded:function(items,position,error){this.itemsPerRow=p.mainView.thumbsPerRow;this.$container.find('.loader').remove();if(!items||!items.length){var msg=null;var fm=null;if(error&&(fm=error.match(/^(nsfw|nsfl|sfw)Required$/))){msg='Das Bild wurde als <em>'+fm[1].toUpperCase()+'</em> markiert.<br/>'+
...
var html=this.buildItemRows(items,0,items.length-numPlaceholders,position);this.$streamContainer.prepend(this.prepareThumbsForInsertion(html));var newHeight=$('#main-view').height()-(117-52);$(document).scrollTop($(document).scrollTop()+(newHeight-prevHeight));}
...
html+='</div>'}
return html;},buildItem:function(item){return('<a class="silent thumb" id="item-'+item.id+'" href="'+this.baseURL+item.id+'">'+'<img src="'+item.thumb+'"/>'+'</a>');},onscroll:function(ev){this.scrollPosition=$(document).scrollTop();if(this.loadInProgress||!this.hasItems){return;}
var loadingTargetNewer=this.loadingTargetDistance/2,loadingTargetOlder=$('#main-view').height()-this.loadingTargetDistance+400;if(this.scrollPosition>loadingTargetOlder&&!this.stream.reached.end){this.loadInProgress=true;this.stream.loadOlder(this.loadedBound);this.$streamContainer.append(p.View.Base.LoadingAnimHTML);}
else if(this.scrollPosition<loadingTargetNewer&&!this.stream.reached.start){this.loadInProgress=true;this.stream.loadNewer(this.loadedBound);this.$container.prepend(p.View.Base.LoadingAnimHTML);$(document).scrollTop(this.scrollPosition+(117-52));}},showItem:function($item,scrollTo){if(this.$currentItem&&$item.is(this.$currentItem)){this.hideItem();this._wasHidden=true;this.currentItemId=null;return;}
this.$currentItem=$item;var $row=$item.parent();var scrollTarget=0;if(scrollTo==this.SCROLL.FULL){scrollTarget=$row.offset().top-CONFIG.HEADER_HEIGHT+$item.height();}
...
if(!$row.next().hasClass('item-container')){if(this.$itemContainer){if(this.$itemContainer.offset().top<$item.offset().top){scrollTarget-=this.$itemContainer.innerHeight()+this.rowMargin*2;}
if(animate){this.$itemContainer.find('.gpt').remove();this.$itemContainer.slideUp('fast',function(){$(this).remove();});}
else{this.$itemContainer.remove();}}
this.$itemContainer=this.$itemContainerTemplate.clone(true);this.$itemContainer.insertAfter($row);if(animate&&!this.jumpToItem){this.$itemContainer.slideDown('fast');}
...
this.currentItemSubview=new p.View.Stream.Item(this.$itemContainer,this);this.currentItemSubview.show(rowIndex,itemData,previousItemHeight,this.jumpToComment);this.jumpToComment=null;this.stream.loadInfo(itemData.id,this.prefetch.bind(this,$item));if(!this.jumpToItem){if(animate){$('body, html').stop(true,true).animate({scrollTop:scrollTarget},'fast');}
else{$('body, html').stop(true,true).scrollTop(scrollTarget);}}
this.currentItemId=id;},hideItem:function(ev){var c=this.$itemContainer;c.slideUp('fast',function(){c.remove();});p.navigateTo(this.baseURL.substr(1,this.baseURL.length-2),p.NAVIGATE.SILENT);this.$itemContainer=null;this.$currentItem=null;this.currentItemSubview.remove();},prefetch:function($centerItem){var $prev=this.getPrevItem($centerItem);var $next=this.getNextItem($centerItem);var queue=[];if($prev.length){queue.push($prev[0].id.replace('item-',''));}
if($next.length){queue.push($next[0].id.replace('item-',''));}
for(var i=0;i<this.prefetched.length;i++){this.prefetched[i].src="";}
this.prefetched=[];for(var i=0;i<queue.length;i++){this.stream.loadInfo(queue[i]);var source=this.stream.items[queue[i]].image;if(source.match(/\.webm$/)){if(p.Video.canPlayWebM){var vid=$('<video autobuffer/>')[0];vid.src=source;this.prefetched.push(vid);}
else{}}
...
this.showItem($prev,this.SCROLL.FULL);p.navigateTo($prev.attr('href').substr(1),p.NAVIGATE.SILENT);return false;}});p.View.FollowList=p.View.Base.extend({template:'<h1 class="pane-head user-head">Stelzliste</h1> <div class="tab-bar"> <a href="#stalk/list/stalk-date" <?js if(params.sort==\'stalk-date\') {?>class="active"<?js}?>>Nach Stelz-Datum</a> <a href="#stalk/list/post-date" <?js if(params.sort==\'post-date\') {?>class="active"<?js}?>>Nach letztem Post</a> </div> <div class="pane"> <?js for( var i = 0; i < list.length; i++ ) { var followed = list[i]; ?> <div class="followed-user"> <?js if( followed.itemId ) { ?> <a class="thumb" href="#user/{followed.name}/uploads/{followed.itemId}"><img src="{followed.thumb}"/></a> <?js } else { ?> <div class="thumb-placeholder"></div> <?js } ?> <div class="follow-details following"> <div> <a href="#user/{followed.name}" class="user um{followed.mark}">{followed.name}</a> </div> <div class="follow-times"> <div> Letzter Post <?js if( followed.lastPost ) {?> <span class="time" title="{followed.lastPost.readableTime()}"> {followed.lastPost.relativeTime()} </span> <?js } else { ?> <span class="time">–</span> <?js } ?> </div> <div> Gestelzt <span class="time" title="{followed.followCreated.readableTime()}"> {followed.followCreated.relativeTime()} </span> </div> </div> <span class="action user-unfollow" data-name="{followed.name}"> <span class="pict">@</span> unstelzen </span> <span class="action user-follow" data-name="{followed.name}"> <span class="pict">@</span> stelzen </span> </div> </div> <?js } ?> </div>',load:function(){p.api.get('user.followlist',{flags:p.user.flags},this.loaded.bind(this));return false;},loaded:function(response){if(this.data.params.sort=='post-date'){response.list.sort(p.View.FollowList.SortFollowLastPost);}
...
this.data.followCount=response.followCount;this.data.dynamicBadges=[];for(var i=0;i<p.View.User.BADGE.COMMENTS.COUNTS.length;i++){var c=p.View.User.BADGE.COMMENTS.COUNTS[i];if(response.commentCount>=c.count){var badge=p.copy(p.View.User.BADGE.COMMENTS.TEMPLATE);badge.description=badge.description.replace(/%c/g,c.count);badge.link='#user/'+response.user.name+'/comments/before/'+this.data.newestCommentTime;badge.extra=c.name;this.data.dynamicBadges.push(badge);break;}}
...
var cc=$(this).attr('class');var markup='<div id="'+id+'_tagsinput" class="tagsinput '+cc+'"><div id="'+id+'_addTag">';if(settings.interactive){markup=markup+'<input id="'+id+'_tag" value="" '+'class="tagsinput-writebox" maxlength="'+settings.maxChars+'" '+'data-default="'+settings.defaultText+'" />';}
...
p.api.post('items.post',ev.target,this.posted.bind(this),this.onError.bind(this));this.renderLoading();return false;},posted:function(response){this.data.posted=!response.error;this.data.error=response.error;this.data.selfPosted=!!response.selfPosted;this.data.item=response.item;this.render();if(response.error=='similar'){this.similarView=new p.View.UploadSimilar(this.$container.find('.upload-similar'),this);this.similarView.show(response.similar);this.$container.find('input[name=checkSimilar]').val(0);}},onError:function(response){return p.setView(p.View.Overlay.Error,response.responseJSON);}});p.View.SecretSanta=p.View.Overlay||{};p.View.SecretSanta=p.View.Base.extend({name:'overlay-secretsanta',template:'<h1 class="pane-head secret-santa">wichteln 2015</h1> <div class="pane secret-santa"> <?js /* not allowed */ if( error === \'notAllowed\' ) { ?> <h2 class="warn">Entschuldige,<br/>Du darfst nicht teilnehmen :/</h2> <p> Am pr0gramm Wichteln dürfen nur nicht-Fliesentischbesitzer teilnehmen, die bereits vor dem 1. Nov 2015 schon mindestens 50 Kommentare oder 10 Uploads hatten. </p> <p>(<a href="{announcementPostLink}">Wichtelrelevante Posts</a>)</p> <?js /* saved */ } else if( saved || (phase == \'confirm\' && confirmed) ) { ?> <h3>Danke!</h3> <p> Am <span class="important"> 6.12.2015</span> erhältst du von <a href="/user/wichtelb0t">@wichtelb0t</a> per privater Nachricht den Benutzernamen und die Anschrift des zu Beschenkenden. </p> <p>(<a href="{announcementPostLink}">Wichtelrelevante Posts</a>)</p> <?js /* confirm */ } else if( phase == \'confirm\' && !confirmed && isInList ) { ?> <form method="post" id="secret-santa-form"> <?js if( error ) { ?> <p class="warn">Da ist was schief gelaufen!</p> <?js } else { ?> <p class="warn"> Bitte bestätige nochmals Deine Teilnahme, damit wir wissen, dass Du es tatsächlich ernst meinst! </p> <?js } ?> <h3>Deine Anschrift</h3> <p> {{firstName}} {{lastName}}<br/> {{packstation}}<br/> {{street}} {{houseNumber}}<br/> {{zip}} {{city}}<br/> {{country}} </p> <div class="form-row" style="width: 400px; padding-top: 24px"> <h3>Teilnahmebedingungen</h3> <p> Deine Anschrift und Dein Benutzername wird einmalig, im Rahmen des pr0gramm-Wichtelns, an einen anderen zufällig ausgewählten Teilnehmer übermittelt. </p> <p> Du erhältst am <span class="important">6.12.2015</span> per Privater Nachricht die Anschrift Deines Dir zugelosten Wichtels und verpflichtest Dich dazu, diesem Teilnehmer bis zum <span class="important">21. Dezember 2015</span> ein kleines Geschenk per Post zu schicken. </p> <p> Du verpflichtest Dich, die Anschrift des anderen Teilnehmers zu keinem anderen Zweck zu verwenden, nicht zu publizieren und keinen sonstigen Mist damit anzustellen. </p> <p> Wir haften nicht für Schäden oder Unannehmlichkeiten, die durch Geschenke Anderer entstehen. </p> <?js if( error === \'mustAcceptTos\' ) { ?> <p class="warn">Du musst die Teilnahmebedingungen akzeptieren!</p> <?js } ?> <input type="checkbox" name="tos" id="secret-santa-tos" value="1"/> <input type="hidden" name="confirm" value="1"/> <label for="secret-santa-tos" class="checkbox-label"> Ich akzeptiere die oben stehenden Teilnahmebedingungen und schwöre auf meine Mutter (optional auch auf den Koran) mich nicht wie ein Arschloch zu verhalten. </label> </div> <div class="form-row" style="padding-top: 24px"> <input type="submit" class="secret-santa-submit" value="Ja Mann, ich will wirklich mitmachen!"/> </div> </form> <?js /* running, show address */ } else if( phase == \'running\' && isInList && partner ) { ?> <h3>Dein Wichtelpartner</h3> <p><a href="/user/{{partner.user}}" class="user um8">{{partner.user}}</a></p> <p> {{partner.firstName}} {{partner.lastName}}<br/> {{partner.packstation}}<br/> {{partner.street}} {{partner.houseNumber}}<br/> {{partner.zip}} {{partner.city}}<br/> {{partner.country}} </p> <p> Bitte verschicke dein Wichtelgeschenk bis spätestens zum <span class="important">21. Dezember 2015</span>! Wer kein Geschenk verschickt, muss mit einer Sperrung rechnen sowie mit einem Ausschluss bei zukünftigen Aktionen. </p> <h3>Du bekommst ein Geschenk von</h3> <p><a href="/user/{{partner2.user}}" class="user um8">{{partner2.user}}</a></p> <h3>Ein paar Tipps zum Wichteln</h3> <ul> <li> Gib die Adresse Deines Wichtelpartners in <a href="https://www.google.de/maps/place/{{partner.street}},+{{partner.zip}}+{{partner.city}}">Google Maps</a> ein und überprüfe ob sie plausibel ist </li> <li> Mache ein paar Fotos Deines Geschenks, bevor Du es verschickst, falls irgendetwas schief läuft </li> <li> Einen OC-Post von Deinem Geschenk zu erstellen ist Aufgabe des Beschenkten – um die Überraschung zu erhalten, solltest Du es nicht selbst posten. Falls Dein Wichtelpartner dieser Aufgabe nicht nachkommt, kannst du später immernoch einen Post erstellen. </li> <li> Markiert euer Päckchen mit den <a href="#top/484451">Wichtel-Paketaufklebern</a>, damit man schon vor dem Auspacken weiß, dass es sich um das Wichtelgeschenk handelt. </li> <li>An Packstationen kann nur via Post/DHL gesendet werden. Es können dort nur Pakete, Päckchen, Groß- und Maxibriefe angenommen werden.</li> <li>Bei Fragen zur Adresse eures Wichtelpartners, fragt ihn bitte selbst.</li> <li>Stuhle nicht ins Päckchen</li> <li>Hinweis: Dein hier angezeigter Wichtelpartner ERHÄLT ein Geschenk von dir. Er selbst beschenkt jemand anderen und du bekommst von einer dritten Person etwas.</li> </ul> <p> Falls es Probleme gibt – und damit meine ich nicht <em>“Ich weiß nicht was ich schenken soll!”</em> – schreibt uns: <a href="#contact">/contact</a> </p> <?js /* closed */ } else if( (phase == \'confirm\' || phase == \'running\') && (!isInList || !partner) ) { ?> <h2 class="warn">Entschuldige,<br/>die Anmeldung ist geschlossen :/</h2> <p> Vielleicht kannst du ja nächstes Jahr mitmachen. </p> <p>(<a href="{announcementPostLink}">Wichtelrelevante Posts</a>)</p> <?js /* register */ } else if( phase == \'register\' ) { ?> <?js if( error ) { ?> <p class="warn">Da ist was schief gelaufen!</p> <?js } if( isInList ) { ?> <h3>Du nimmst am Wichteln teil!</h3> <p> In <span class="important">der ersten Dezemberwoche 2015</span> erhältst du von <a href="/user/wichtelb0t">@wichtelb0t</a> per privater Nachricht die Anschrift des zu Beschenkenden. </p> <p> Hier kannst Du Deine Anschrift ändern oder Dich aus der Wichtelliste austragen, falls Du es dir doch anders überlegt hast. </p> <?js } else { ?> <p> <span class="important">Lies diese Seite KOMPLETT durch bevor wieder dumme vermeidbare Fragen kommen.</span> </p> <p> Beim Wichteln schickst Du einem zufällig ausgewählten Wichtel-Teilnehmer ein kleines Geschenk. Im Gegenzug erhältst Du von einem anderen zufällig ermittelten Teilnehmer selbst ein Geschenk. Um es nochmal f&uuml;r die Langsamen klar zu stellen, <span class="important">du beschenkst nicht die gleiche Person die dich beschenkt!</span> </p> <p> Die Geschenke können irgendwas selbst Gebasteltes oder Gemaltes sein, oder einfach eine gekaufte Kleinigkeit. Wir empfehlen so um die 15€ für das Geschenk auszugeben. Wer Dinge verschickt um anderen zu schaden oder gar nichts versendet muss mit einer Sperrung seines Accounts rechnen. </p> <p> Wenn du am pr0gramm Wichteln teilnehmen möchtest, trage einfach unten deine Anschrift ein, die wir dann einem einzelnen anderen Teilnehmer zulosen. </p> <p> Deine Anschrift wird von uns zu keinem anderen Zweck verwendet und wird nach Ende des Wichtelns restlos von unseren Servern gelöscht. </p> <p> Alle <span class="important">Packstation Nutzer</span> können in das Postnummer Feld ihre paket.de Nummer eintragen. Als Straße &quot;Packstation&quot; eintragen und als Hausnummer die Nummer der Packstation angeben. </p> <p> Alle <span class="important">Aluhuttr&auml;ger</span> können beim Namen den Vornamen abkürzen. Also z.B. &quot;L. Simpson&quot;. </p> <?js } ?> <p>(<a href="{announcementPostLink}">Wichtelrelevante Posts</a>)</p> <form method="post" id="secret-santa-form"> <h3>Deine Anschrift</h3> <?js if( error === \'incomplete\' ) { ?> <p class="warn">Gib Deine vollständige Anschrift an!</p> <?js } ?> <div class="form-row"> <input type="text" name="firstName" placeholder="Vorname" value="{{firstName}}" style="width:198px"/> <input type="text" name="lastName" placeholder="Nachname" value="{{lastName}}" style="width:198px"/> </div> <div class="form-row"> <input type="text" name="packstation" placeholder="Postnummer oder Adresszusatz (optional)" value="{{packstation}}" style="width:400px"/> </div> <div class="form-row"> <input type="text" name="street" placeholder="Straße" value="{{street}}" style="width:348px"/> <input type="text" name="houseNumber" placeholder="Nr" value="{{houseNumber}}" style="width:50px"/> </div> <div class="form-row"> <input type="text" class="number" name="zip" placeholder="PLZ" value="{{zip}}" style="width:64px"/> <input type="text" name="city" placeholder="Ort" value="{{city}}" style="width:334px"/> </div> <div class="form-row"> <select name="country" class="secret-santa-country"> <option <?js print(country===\'Deutschland\'?\'selected\':\'\');?> value="Deutschland">Deutschland</option> <option <?js print(country===\'Österreich\'?\'selected\':\'\');?> value="Österreich">Österreich</option> <option <?js print(country===\'Schweiz\'?\'selected\':\'\');?> value="Schweiz">Schweiz</option> </select> <p> (Ihr wichtelt nur mit euren eigenen Landsmännern. Österreicher werden nur mit anderen Österreichern wichteln, Schweizer nur mit anderen Schweizern.) </p> </div> <div class="form-row" style="width: 400px; padding-top: 24px"> <h3>Teilnahmebedingungen</h3> <p> Deine Anschrift und Dein Benutzername wird einmalig, im Rahmen des pr0gramm-Wichtelns, an einen anderen zufällig ausgewählten Teilnehmer übermittelt. </p> <p> Du erhältst in <span class="important">am 6.12.2015</span> per privater Nachricht die Anschrift Deines Dir zugelosten Wichtels und verpflichtest Dich dazu, diesem Teilnehmer bis zum <span class="important">21. Dezember 2015</span> ein kleines Geschenk per Post zu schicken. </p> <p> Du verpflichtest Dich, die Anschrift des anderen Teilnehmers zu keinem anderen Zweck zu verwenden, nicht zu publizieren und keinen sonstigen Mist damit anzustellen. </p> <p> Wir haften nicht für Schäden oder Unannehmlichkeiten, die durch Geschenke Anderer entstehen. </p> <?js if( error === \'mustAcceptTos\' ) { ?> <p class="warn">Du musst die Teilnahmebedingungen akzeptieren!</p> <?js } ?> <input <?js print(isInList?\'checked\':\'\');?> type="checkbox" name="tos" id="secret-santa-tos" value="1"/> <label for="secret-santa-tos" class="checkbox-label"> Ich akzeptiere die oben stehenden Teilnahmebedingungen und schwöre auf meine Mutter (optional auch auf den Koran) mich nicht wie ein Arschloch zu verhalten. </label> </div> <div class="form-row" style="padding-top: 24px"> <?js if( isInList ) { ?> <input type="submit" class="secret-santa-submit" value="Änderungen Speichern"/> <input type="button" class="secret-santa-delete cancel" value="Aus der Liste löschen"/> <?js } else { ?> <input type="submit" class="secret-santa-submit" value="Am Wichteln teilnehmen"/> <?js } ?> </div> </form> <?js } ?> </div>',data:{phase:'register',error:null,saved:false,isInList:false,firstName:'',lastName:'',street:'',houseNumber:'',packstation:'',zip:'',city:'',country:'',confirmed:0,announcementPostLink:'#top/1050334'},animationInterval:0,load:function(){p.api.get('secretsanta.info',{},this.loaded.bind(this),this.error.bind(this));return false;},remove:function(){clearInterval(this.animationInterval);},error:function(){this.loaded({error:'notAllowed'});},loaded:function(response){this.data.error=response.error;if(response.entry){this.data=p.merge(this.data,response.entry);this.data.isInList=true;}
this.data.partner=response.partner;this.data.partner2=response.partner2;this.data.phase=response.phase;this.render();var snowDivHTML="";for(var i=0;i<7;i++){snowDivHTML+='<div class="snowflake">*</div>';}
var $snowDivs=$(snowDivHTML);this.$container.append($snowDivs);var w=this.$container.width(),h=this.$container.height();this.flakes=[];for(var i=0;i<$snowDivs.length;i++){var div=$snowDivs[i];var x=w*Math.random(),y=h*Math.random();div.style.left=x+'px';div.style.top=y+'px';this.flakes.push({x:x,y:y,div:div,s:0.5+Math.random()*0.75});}
this.animationInterval=setInterval(this.animate.bind(this),16);},animate:function(){var mh=this.$container.height()-34;for(var i=0;i<this.flakes.length;i++){var flake=this.flakes[i];flake.y+=flake.s;if(flake.y>mh){flake.y=0;}
var x=flake.x+Math.sin((flake.y+i*37)/100)*50;flake.div.style.top=flake.y+'px';flake.div.style.left=x+'px';}},render:function(){this.parent();this.$container.find('#secret-santa-form').submit(this.submit.bind(this));this.$container.find('.secret-santa-delete').fastclick(this.removeList.bind(this));},submit:function(ev){if(this.data.phase=='register '){this.data.firstName=this.$container.find('input[name=firstName]').val();this.data.lastName=this.$container.find('input[name=lastName]').val();this.data.street=this.$container.find('input[name=street]').val();this.data.houseNumber=this.$container.find('input[name=houseNumber]').val();this.data.packstation=this.$container.find('input[name=packstation]').val();this.data.zip=this.$container.find('input[name=zip]').val();this.data.city=this.$container.find('input[name=city]').val();this.data.country=this.$container.find('select[name=country]').val();}
p.api.post('secretsanta.save',ev.target,this.posted.bind(this));this.renderLoading();return false;},removeList:function(response){if(confirm('Willst Du Dich wirklich aus der Wichtelliste löschen?')){p.api.post('secretsanta.remove',{},p.reload);}},posted:function(response){this.data.error=response.error;this.data.saved=response.saved;this.render();}});p.api=new p.ServerAPI(CONFIG.API.ENDPOINT);p.user=new p.User();p.hotkeys=new p.Hotkeys($(document));p.mainView=null;if(p.getLocation().match(/^newest/)){document.location='/';}
if(p.getLocation().match(/^bm\//)){p.mainView=new p.View.Bookmarklet.Layout('body');p.addRoute(p.View.Bookmarklet.Login,'bm/login');p.addRoute(p.View.Bookmarklet.Post,'bm/post/<type>/<siteUrl>/<imageUrl>');}
else{p.mainView=new p.View.Layout('body');p.addRoute(p.View.Stream.Main,'');p.addRoute(p.View.Stream.Main,'<tab:top|new|stalk>');p.addRoute(p.View.Stream.Main,'<tab:top|new|stalk>/<itemId:d>');p.addRoute(p.View.Stream.Main,'<tab:top|new>/<tags>');p.addRoute(p.View.Stream.Main,'<tab:top|new>/<tags>/<itemId:d>');p.addRoute(p.View.Stream.Main,'user/<userName>/<userTab:likes|uploads>');p.addRoute(p.View.Stream.Main,'user/<userName>/<userTab:likes|uploads>/<itemId:d>');p.addRoute(p.View.User,'user/<name>');p.addRoute(p.View.User.Comments,'user/<name>/comments/<search:after|before>/<timestamp:d>');p.addRoute(p.View.Inbox,'inbox/<tab:unread|all|messages>');p.addRoute(p.View.Inbox,'inbox/<tab:unread|all|messages>/<search:after|before>/<timestamp:d>');p.addRoute(p.View.Validate,'user/<name>/validate/<token>');p.addRoute(p.View.ResetPassword,'user/<name>/resetpass/<token>');p.addRoute(p.View.FollowList,'stalk/list/<sort:stalk-date|post-date>');p.addRoute(p.View.Settings,'settings/account/mail/<token>');p.addRoute(p.View.Settings,'settings/<tab:bookmarklet|site|account|invites>');p.addRoute(p.View.Upload,'upload');p.addRoute(p.View.Contact,'contact');p.addRoute(p.View.RegisterInvite,'register/<token>');p.addRoute(p.View.RedeemThanks,'redeem/thanks');p.addRoute(p.View.RedeemCode,'redeem/<token>');p.addRoute(p.View.BuyCode.PaypalThanks,'pr0mium/thanks');p.addRoute(p.View.BuyCode.Products,'pr0mium');p.addRoute(p.View.FAQ,'faq');p.addRoute(p.View.Imprint,'imprint');p.addRoute(p.View.TOS,'tos');p.addRoute(p.View.SecretSanta,'secret-santa');}
p.addRoute(p.View.Error404,'*');p.mainView.show();p.start('#main-view');if(CONFIG.ANALYTICS.ENABLED){(function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){(i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),m=s.getElementsByTagName('head')[0];a.async=1;a.src=g;m.appendChild(a)})(window,document,'script','//www.google-analytics.com/analytics.js','ga');ga('create',CONFIG.ANALYTICS.ACCOUNT,'auto');ga('set','dimension1',(p.user.id?'yes':'no'));ga('set','dimension2',p.user.flagsName);ga('send','pageview',{page:p.getLocation()});}
